...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    if (!nameRegex.test(req.body.name)) throw new Error("invalid name");
    if (!emailRegex.test(req.body.email)) throw new Error("invalid email");
    ...
    await mailServices.sendVerifyMail(
      newUser.userName,
      newUser.email,
      emailToken
    );
    ...
    if (user !== null) {
      if (bcrypt.compareSync(req.body.password, user.hash_password)) {
        ...
        await redisClient.setEx(
          token,
          Number(process.env.TOKEN_DAY_LIFE) * 86400,
          token
        );
        return res.json({
          status: 200,
          resultData: { ...userInfo, jwt: token },
        });
      } else {
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    return res.json({
      status: 404,
      errorMessage: "can't find user of this email",
    });
  } catch (err: any) {
    console.log(err);
    return res.json({ status: 404, errorMessage: err.message });
...
export const updateUserInfo = async (req: Request, res: Response) => {
  try {
    ...
    if (user) {
      const nameRegex = new RegExp("^[A-Za-z0-9 ]+$", "g");
      const emailRegex = new RegExp("^(.+)@(.+)$");
      if (!nameRegex.test(req.body.userName)) throw new Error("invalid name");
      if (!emailRegex.test(req.body.email)) throw new Error("invalid email");
      ...
      } else if (req.body?.avatar) user.avatar = req.body.avatar;
